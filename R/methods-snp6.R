...
old.genotypeOne <- function(files, tmpdir=getwd(), batch_size=40000,
  ...
  if (!file.exists(tmpdir)){
    ...
    stop("Directory ", tmpdir, " already exists. Cannot use it.")
  }
...
##   conf.file <- gzfile(file.path(tmpdir, "crlmm-conf.txt.gz"), "w")
  ...
  for (i in 1:n.chunks){
    ...
    #############
    #############
    fileParsCentersBefore <- file.path(tmpdir, "centersBefore.txt")
    fileParsScalesBefore <- file.path(tmpdir, "scalesBefore.txt")
    fileParsNBefore <- file.path(tmpdir, "nBefore.txt")
    ...
    ## Temporarily saving
    fileDistBeforeAA <- file.path(tmpdir, "distBeforeAA.txt")
    suppressWarnings(write.table(myDist[,, 1], fileDistBeforeAA, append=TRUE,
                                 quote=FALSE, sep="\t", col.names=(i==1)))
    fileDistBeforeAB <- file.path(tmpdir, "distBeforeAB.txt")
    suppressWarnings(write.table(myDist[,, 2], fileDistBeforeAB, append=TRUE,
                                 quote=FALSE, sep="\t", col.names=(i==1)))
    fileDistBeforeBB <- file.path(tmpdir, "distBeforeBB.txt")
    ...
    if (recalibrate){
      ...
      #############
      #############
      fileParsCentersAfter <- file.path(tmpdir, "centersAfter.txt")
      fileParsScalesAfter <- file.path(tmpdir, "scalesAfter.txt")
      fileParsNAfter <- file.path(tmpdir, "nAfter.txt")
      ...
      dimnames(move) <- list(tmpdf[["man_fsetid"]][overall_pos],
                             c("AA", "AB", "BB"))
      fileMove <- file.path(tmpdir, "move.txt")
      ...
      fileDistAfterAA <- file.path(tmpdir, "distAfterAA.txt")
      suppressWarnings(write.table(myDist[,, 1], fileDistAfterAA, append=TRUE,
                                   quote=FALSE, sep="\t", col.names=(i==1)))
      fileDistAfterAB <- file.path(tmpdir, "distAfterAB.txt")
      suppressWarnings(write.table(myDist[,, 2], fileDistAfterAB, append=TRUE,
                                   quote=FALSE, sep="\t", col.names=(i==1)))
      fileDistAfterBB <- file.path(tmpdir, "distAfterBB.txt")
...
old.normalizeOne <- function(celFiles, destDir, batch_size=40000, verbose=TRUE, pkgname, reference=TRUE){
