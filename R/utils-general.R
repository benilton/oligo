...
getProbeIndex <- function(object, probeType='pm', target='core', subset, sortBy='fid'){
    ...
    if (class(object) %in% c('ExonFeatureSet', 'GeneFeatureSet')){
        ## ST Arrays
        info <- switch(target,
               core=getFidMetaProbesetCore(object, NULL),
               full=getFidMetaProbesetFull(object, NULL),
               extended=getFidMetaProbesetExtended(object, NULL),
               probeset=getFidProbeset(object, NULL))
