...
##  celFiles <- dir(origin, pattern=".CEL", full.names=TRUE)
  ...
  if (verbose) message("Preparing environment for normalization."); t0 <- proc.time()
##  sapply(unlist(batches.out), function(x) suppressWarnings(createCel(x, header=header)))
