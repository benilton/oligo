## RMA here is simply done to the probeset-level
## b/c the db structure is different (1:N mapping from probe to probeset)
## I'm reimplementing the method for RMA/Selecting the probes

setMethod("rma", "GeneFeatureSet",
          function(object, background=TRUE, normalize=TRUE, subset=NULL){
            conn <- db(object)

            ## This is the only difference from RMA for Exon
            sql <- paste("SELECT fid, fsetid FROM pmfeature INNER JOIN f2fset USING(fid)")
