...
/*
Trying to read gzipped XYS files
*/

static int isgzXysFile(const char *filename){
 gzFile currentFile = NULL; 
 char buffer[LINEMAX];
 char *token;
 currentFile = gzopen(filename, "r");
 if (currentFile == NULL){
   error("Could not open file %s", filename);
 } else {
   /** check if file starts with "# software=NimbleScan" **/
   gzgets(currentFile, buffer, LINEMAX);
   gzgets(currentFile, buffer, LINEMAX);
 ...
 ** countLines: counts lines, returns integer
***************************************************************/
static int countLinesGz(gzFile *file){
  int lines = 0;
  char buffer[LINEMAX];
  while (!gzeof(file)){
...
static void read_one_gzxys(const char *filename, double *signal,
			   int *xy, int i, int nrow, int verbose){
  int count;
  //  char sx[WORD], sy[WORD], ss[WORD], sc[WORD], *endpx, *endpy, *endps;
  char buffer[LINEMAX];
  ...
  while (!gzeof(fp)){
    ...
    if (token == NULL)
      error("Incomplete line found.");
    if (token[0] != 'N'){
      signal[count + i*nrow] = atof(token);
    }else{ // NA_REAL is R-specific
      signal[count + i*nrow] = NA_REAL;
    }
...
SEXP R_read_gzxys_files(SEXP filenames, SEXP verbosity){
  ...
  verbose = asLogical(verbosity);
  nfiles = length(filenames);
  fp = gzopen(CHAR(STRING_ELT(filenames, 0)), "r");
  if (fp == NULL)
    error("Can't open %s.\n", CHAR(STRING_ELT(filenames, 0)));
  nrows = countLinesGz(fp)-2;
  gzclose(fp);
  ...
  // Parsing files here
  for (i=0; i < nfiles; i++){
    Rprintf("Detected as GZ: %d\n", isgzXysFile(CHAR(STRING_ELT(filenames, i))));
    read_one_gzxys(CHAR(STRING_ELT(filenames, i)), ptr2signal,
		 ptr2xy, i, nrows, verbose);
