...
rownames(aboutSamples) <- sampleNames(preProcessedData)
aboutVars <- data.frame(labelDescription="male/female")
rownames(aboutVars) <- "gender"
phenoData(preProcessedData) <- new("AnnotatedDataFrame",
                                   data=aboutSamples,
...
<<>>=
annot <- annotation(preProcessedData)
require(annot, character.only=TRUE)
conn <- oligo:::db(get(annot))
dbListTables(conn)
