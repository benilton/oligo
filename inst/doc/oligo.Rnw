...
%\VignetteDepends{oligo, hapmap100kxba, pd.mapping50k.xba240}
...
<<theCalls>>=
calls.file <- file.path(outputDir, "crlmm-calls.txt")
conf.file <- file.path(outputDir, "crlmm-conf.txt")
read.delim(calls.file, nrows=5)[, 1:2]
read.delim(conf.file, nrows=5)[, 1:2]
@ 
